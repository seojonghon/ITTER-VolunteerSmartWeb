...
- 서브넷 2개생성 한개는 백엔드<스프링부트 배포용)뷰 배포용(마지막에서 안됨)
- 서브넷 각각에 각자의 인스턴스 생성
- 각각 보안그룹을 연결해줌
-  백엔드 인스턴스에 접속해서 깃허브에 있는 레파지터리 클론해서 가져옴
- Jar파일 형태로 변환후 배포진행
- 배포진행을 위해 기존 mria db를 RDS로 변경
<-클라우드 관계형Db 즉가상머신 위에서 db를 띄움
- Db에 접속할 수 있는 엔드포인트가 나오게 되고 이를통해 백엔드 코드에 엔드포인트를 삽입
- 배포후 인스턴스의 퍼블릭 ip주소가 할당되고 이 주소를 로컬호스트 대신에 프론트에서 입력후 접속
### AWS
